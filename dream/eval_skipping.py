...
class Dream(LM):
    def __init__(
        ...
        **kwargs,
        ...
        skip_layers_arg = kwargs.get("skip_layers", None)
        ...
        # ---- Static skip layer support ----
        self.static_skips = set()
        if skip_layers_arg:
            try:
                self.static_skips = set(map(int, str(skip_layers_arg).split("|")))
                print(f"[INFO] Static skipping layers: {sorted(self.static_skips)}")
            except Exception as e:
                print(f"[WARN] Could not parse skip_layers={skip_layers_arg}: {e}")

        if self.static_skips:
            for i, block in enumerate(blocks):
                if i in self.static_skips:
                    def skip_forward(*args, **kwargs):
                        x = args[0]
                        return x, None  # return tuple for unpacking safety
